...
class _HomePageState extends State<HomePage> {
  ...
  void _updateCheckinState(List<MoodCheckinModel> checkins) {
    final today = DateTime.now();
    final todayStart = DateTime(today.year, today.month, today.day);
    final todayEnd = DateTime(today.year, today.month, today.day, 23, 59, 59);
    
    final todayCheckins = checkins.where((checkin) {
  ...
  Future<void> _loadData() async {
    ...
    // 1. Coba load data dari cache lokal terlebih dahulu
    bool hasCache = false;
    try {
      final cachedDashboardJson = await LocalStorageService.getDashboard();
      final cachedCheckinsJson = await LocalStorageService.getCheckinHistory();
      ...
      if (cachedDashboardJson != null && cachedCheckinsJson != null) {
        ...
        _updateCheckinState(cachedCheckins);
        
        hasCache = true;
        setState(() {
          _isLoading = false; // Ada cache, langsung tampilkan UI
        });
      }
    } catch (e) {
    ...
    // 2. Fetch data dari API Laravel di background
    try {
      final dashboardResult = await _apiService.getDashboard();
      final todayCheckinResult = await _apiService.getCheckinHistory();

      if (dashboardResult['success'] && todayCheckinResult['success']) {
        final dashboard = dashboardResult['data'] as DashboardModel;
        final checkins = todayCheckinResult['data'] as List<MoodCheckinModel>;

        _updateDashboardState(dashboard);
