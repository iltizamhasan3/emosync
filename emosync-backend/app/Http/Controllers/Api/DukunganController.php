    ...
    public function index(Request $request)
    {
        ...
        $kritis = MoodCheckin::where('user_id', $user->id)
            ...
            ->where(function ($query) {
                $query->where('kuadran', 'low_energy_unpleasant')
                      ->orWhere('kuadran', 'high_energy_unpleasant');
            })
            ->count();
        ...
        } elseif (in_array($latest->kuadran, ['low_energy_unpleasant', 'high_energy_unpleasant'])) {
            $level = 'ringan';
        ...
        return response()->json([
            'level' => $level,
            'message' => $message,
            'checkin_terakhir' => $latest->kuadran,
